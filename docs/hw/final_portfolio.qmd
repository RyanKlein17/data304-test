...
```{python}
mil_burden_data.at[251, 'Country Name'] = "United States of America"
mil_burden_data.at[203, 'Country Name'] = "Russia"
mil_burden_data.at[112, 'Country Name'] = "Iran"
mil_burden_data.at[43, 'Country Name'] = "Dem. Rep. Congo"
```
...
country_map.properties(width = 1000, height = 750)
```
...
I am currently taking POLS 217 (International Cooperation and Conflict). Our professor noted that there were no recent maps on the internet that displayed the defense burden of each country. After finding a dataset collected and compiled by the World Bank that contained this metric for many countries, we decided to create a chlorepleth world map and map the defense burden of each country in 2022 to each country. The quantitative color scale maps out what percent of the country's overall GDP was spent on their military. Most of the countries spent less than 10% of their GDP on their military, while Ukraine spent more than 30% of their GDP on their military. I will be perfecting and continuing to update this map as time allows, but presently, this is the final version for the portfolio.

### Part 5

1.  an encoding channel other than x or y.

    -   In part 2 of this portfolio, we mapped the variable region to the color channel.

2.  layers

    -   The world map depicts layering, as the second map is layered with two maps. The bottom map has a basic outline of the map of the world with all of the country boundaries drawn in. The second layer fills in the countries with color if there is data for that country. However, the underlying layer is still there.

3.  concatenation

    -   Concatenation can be seen in the comparing kits plot, where we implemented a horizontal concat.

4.  non-default configuration of a channel’s scale or guide

    -   One example of adjustments made to the scale was in the "Comparing Twins" chart, where we see the implementation of XOffset. Another example was in the gas chart, where we see that the legend was set to None. This was done to prevent confusion when only one of the values of the variable was selected to show.

5.  interaction (tooltips, panning/zooming, brushing, etc.)

    -   We used several tooltips. One example is in the map, where we added a tooltip to show the name of the country and the country's defense burden when the cursor hovered over the country.
